...
# --- Ensure values are always in session state ---
...
# --- Reports & Downloads ---
    ...
    if st.session_state.assigned_duty_df is not None and not st.session_state.assigned_duty_df.empty:
        ...
        # --- Keep Record Button for MasterLoad.xlsx ---
        st.markdown("---")
        st.subheader("Master Load Record")
        if st.button("Keep Record in MasterLoad.xlsx"):
            import openpyxl
            masterload_path = "MasterLoad.xlsx"
            # Get exam name for column (use actual values)
            exam_type = st.session_state.get('exam_type', 'EXAM')
            semester = st.session_state.get('semester', 'SEM')
            year = st.session_state.get('year', 'YEAR')
            # --- FIX: Use values, not literal string ---
            exam_col = f"{exam_type} {semester} {year}"
            # Calculate current load per faculty
            faculty_list = st.session_state.faculty_df['faculty'].astype(str).str.strip().tolist()
            load_dict = {f: 0 for f in faculty_list}
            for faculty in faculty_list:
                load_dict[faculty] = len(df[df['Faculty'] == faculty])
            # Read or create MasterLoad.xlsx, robust to missing/misnamed Faculty column
            if os.path.exists(masterload_path):
                mdf = pd.read_excel(masterload_path, engine="openpyxl")
                # Try to find the Faculty column, case-insensitive
                faculty_col = None
                for col in mdf.columns:
                    if col.strip().lower() == "faculty":
                        faculty_col = col
                        break
                if faculty_col is None:
                    # If not found, treat as new
                    mdf = pd.DataFrame({"Faculty": faculty_list, "Total Load": [0]*len(faculty_list)})
                    faculty_col = "Faculty"
                else:
                    # Rename to standard
                    mdf = mdf.rename(columns={faculty_col: "Faculty"})
            else:
                mdf = pd.DataFrame({"Faculty": faculty_list, "Total Load": [0]*len(faculty_list)})
            # Ensure all faculty are present (no duplicates, normalize names)
            mdf['Faculty'] = mdf['Faculty'].astype(str).str.strip()
            faculty_set = set(mdf['Faculty'].str.lower())
            for f in faculty_list:
                f_norm = f.strip()
                if f_norm.lower() not in faculty_set:
                    mdf = pd.concat([mdf, pd.DataFrame({"Faculty": [f_norm], "Total Load": [0]})], ignore_index=True)
                    faculty_set.add(f_norm.lower())
            # Insert new exam column before 'Total Load'
            if exam_col in mdf.columns:
                st.warning(f"Column '{exam_col}' already exists. Overwriting values.")
            else:
                total_idx = mdf.columns.get_loc("Total Load")
                mdf.insert(total_idx, exam_col, 0)
            # Fill in the new exam column
            for idx, row in mdf.iterrows():
                faculty = row["Faculty"]
                mdf.at[idx, exam_col] = load_dict.get(faculty, 0)
            # Recalculate Total Load (sum of all assignment columns that match the pattern, never phone numbers)
            assignment_col_pattern = re.compile(r'^(MID|END) SEM (MO|SP) \d{4}$', re.IGNORECASE)
            assign_cols = [col for col in mdf.columns if assignment_col_pattern.match(str(col))]
            mdf[assign_cols] = mdf[assign_cols].apply(pd.to_numeric, errors='coerce')
            mdf["Total Load"] = mdf[assign_cols].sum(axis=1)
            # Save back to Excel
